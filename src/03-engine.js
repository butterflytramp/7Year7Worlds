...
const KEYS=[
 [0.000, 22, 17,  .00, .00],   /* wide entrance chamber */
 [0.100, 20, 15,  .12, .05],
 [0.150, 5.6, 6.4, .00, .02],  /* narrow passage */
 [0.230, 6.2, 7.0,-.08, .03],
 [0.280, 15, 26,  .18, .05],   /* tall atrium */
 [0.390, 14, 27,  .08, .05],
 [0.440, 9.5, 9.6, .00, .04],  /* curving gallery */
 [0.590, 10, 9.2, -.14, .04],
...
seams[1].set(2.3, 0, S(.15), S(.24));                   /* narrow passage, upper left */
seams[2].set(.62, 0, S(.298), S(.362));                 /* atrium — the bar of a 7 */
seams[3].set(-.72, (0.62+.72)/4.2, S(.362)-4.2, S(.362)); /* — and its stroke */
...
addPool(S(.335),-Math.PI/2, 6, .8, 0xfff5e6, .20);
...
function niche(w, g, hw, hh, depth){
  ...
  const dk=new THREE.MeshBasicMaterial({color:0x090b12});
...
function makeWorld(w){
  ...
  if(w.type==='wall'){
    ...
    const depth=w.open?4.2:1.6;
    const back=niche(W,g,w.rs,w.rc,depth);
    if(w.open){
      const tex=w.glimpse();
      const pic=new THREE.Mesh(new THREE.PlaneGeometry(w.rs*2+2.2,w.rc*2+2.2),
...
/* World 05 — a giant ribbon curves around the entrance */
{
  const W=WORLDS[4], w=W.def;
  const P0=W.point, N=W.normal, Tg=new THREE.Vector3().crossVectors(UP,N).normalize();
  const M=90, pos=[], idx=[]; const R=w.rs+2.6;
  for(let k=0;k<=M;k++){
    const th=-.55*Math.PI+ (2.05*Math.PI)*k/M;
    const cs=Math.cos(th), sn=Math.sin(th);
    const lift=1.3+1.4*Math.sin(th*1.5+1.0)+ .9*(k/M);
    const c=P0.clone().addScaledVector(Tg,R*cs).addScaledVector(UP,R*sn*1.1+ w.c*0 ).addScaledVector(N,lift);
    const rad=Tg.clone().multiplyScalar(cs).addScaledVector(UP,sn).normalize();
    const wid=.9+.5*Math.sin(th*2);
    const a=c.clone().addScaledVector(rad,-wid), b=c.clone().addScaledVector(rad,wid);
    pos.push(a.x,a.y,a.z,b.x,b.y,b.z);
    if(k<M){ const q=k*2; idx.push(q,q+1,q+2, q+1,q+3,q+2); }
  }
  const g=new THREE.BufferGeometry(); g.setAttribute('position',new THREE.Float32BufferAttribute(pos,3)); g.setIndex(idx); g.computeVertexNormals();
  scene.add(new THREE.Mesh(g, whiteMat));
}
...
hangArt('a2.jpg',0.8,2.5,.035, 1);
hangArt('a1.jpg',1.667,1.9,.048,-1);
hangArt('truck.jpg',1.499,2.0,.072,-1);
hangArt('cricket.jpg',1.664,2.1,.088, 1);
hangArt('a9.jpg',1.57,1.5,.188,-1,2.2);
hangArt('a3.jpg',0.8,1.8,.212, 1,2.2);
hangArt('paperbag.jpg',1.687,2.2,.445,-1);
hangArt('heroslider.jpg',1.723,2.0,.458, 1);
hangArt('a4.jpg',0.8,2.2,.468,-1);
hangArt('solvecube.jpg',1.778,1.9,.482, 1);
hangArt('a11.jpg',0.821,2.4,.492,-1);
hangArt('a13.jpg',0.984,2.2,.53,-1);
hangArt('a14.jpg',0.999,2.2,.536, 1);
hangArt('layer.jpg',1.777,1.7,.548,-1);
hangArt('sloka.jpg',0.8,2.6,.556, 1);
hangArt('a10.jpg',0.797,2.4,.566,-1);
hangArt('frame-a.jpg',1.6,2.0,.575, 1);
hangArt('a6.jpg',1.048,3.0,.585,-1,2.9);   /* hero — the mural */
hangArt('a15.jpg',1.778,1.8,.596, 1);
hangArt('a5.jpg',0.8,2.0,.606,-1);
hangArt('cherrypickers.jpg',1.599,2.2,.615, 1);
hangArt('a7.jpg',1.667,1.9,.884,-1);
hangArt('frame-b.jpg',1.6,2.1,.898, 1);
hangArt('frame-c.jpg',1.646,2.0,.912,-1);
hangArt('a12.jpg',0.71,2.4,.926, 1);
hangArt('a8.jpg',0.784,2.5,.94,-1);
...
function makeSound(){
  ...
  const airG=AC.createGain(); airG.gain.value=.5; noise.connect(lp); lp.connect(airG); airG.connect(master); noise.start();
  /* structural hum */
  const hum=AC.createOscillator(); hum.frequency.value=52; const humG=AC.createGain(); humG.gain.value=.045; hum.connect(humG); humG.connect(master); hum.start();
  const hum2=AC.createOscillator(); hum2.frequency.value=78.3; const hum2G=AC.createGain(); hum2G.gain.value=.02; hum2.connect(hum2G); hum2G.connect(master); hum2.start();
...
function frame(){
  ...
  if(trans&&trans.W.def.kind==='chrome'){ camera.rotateZ(.12*Math.sin(clamp((t-trans.t0)/trans.dur,0,1)*Math.PI)); }
  ...
  for(const W of WORLDS){
    ...
    if(W.slab){ const want=(hovered===W.slab)?1.04:1; W.slab.scale.x+=(want-W.slab.scale.x)*.1; W.slab.scale.y+=(want-W.slab.scale.y)*.1; }
  }
