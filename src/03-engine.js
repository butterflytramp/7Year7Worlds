...
/* ============ the end of the building ============
   ...
   so the arch at the end of the tube is the work itself. UVs are built from
   each vertex's position in the section's own plane and cover-fitted, so the
   image fills the arch without stretching. */
{
  ...
  const R=_e.R.clone().normalize();
  const ring=pts.map(q=>q.clone().sub(C).multiplyScalar(1.9).add(C));
  ...
  const W7=WORLDS.find(x=>x.def.id==='07');
  if(W7){ cap.userData={kind:'sealed',W:W7}; clickables.push(cap); W7.slab=cap;
    W7.point=C.clone().addScaledVector(_e.T,6); W7.normal=_e.T.clone().negate(); }
}
